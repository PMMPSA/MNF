			...
			if(!($sender instanceof Player)){
				...
				if($sender->hasPermission("sell") || $sender->hasPermission("sell.hand") || $sender->hasPermission("sell.all")){
					...
					if(isset($args[0]) && strtolower($args[0]) == "hand"){
						...
						$sender->sendMessage(TF::GREEN . TF::BOLD . "§8(§a!§8) " . TF::RESET . TF::GREEN . "$" . $price . " đã nhập vô số tiền của tiền của bạn.");
					...
					/* Sell All */
						...
						foreach($items as $item){
							if($this->sell->get($item->getId()) !== null && $this->sell->get($item->getId()) > 0){
								...
								$sender->sendMessage(TF::GREEN . TF::BOLD . "§8(§a!§8) " . TF::RESET . TF::GREEN . "Bạn đã nhận được " . TF::RED . "$" . $price . TF::GREEN . " khi bán (" . $item->getCount() . " " . $item->getName() . " với $" . $this->sell->get($item->getId()) . " với mỗi).");
								$sender->getInventory()->remove($item);
							}
						}
					}elseif(isset($args[0]) && strtolower($args[0]) == "about"){
						$sender->sendMessage(TF::RED . TF::BOLD . "§8(§a!§8) " . TF::RESET . TF::GRAY . "Server sử dụng plugin Sell Hand, bởi Muqsit Rayyan và sửa bởi georgianYT.");
					}else{
						$sender->sendMessage(TF::RED . TF::BOLD . "§8(§a!§8) " . TF::RESET . TF::DARK_RED . "§bCửa hàng bán đồ online");
